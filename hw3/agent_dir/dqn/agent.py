...
from agent_dir.dqn.base import BaseModel
from agent_dir.dqn.history import History
from agent_dir.dqn.replay_memory import ReplayMemory
from agent_dir.dqn.ops import linear, conv2d, clipped_error
from agent_dir.dqn.utils import get_time, save_pkl, load_pkl
...
class Agent(BaseModel):
    ...
    def predict(self, s_t, test_ep=None):
        ...
        if random.random() < ep:
            action = random.randrange(self.env.get_action_space().n)
        else:
    ...
    def build_dqn(self):
        ...
        with tf.variable_scope('prediction'):
            ...
            if self.dueling:
                ...
                self.advantage, self.w['adv_w_out'], self.w['adv_w_b'] = \
                    linear(self.adv_hid, self.env.get_action_space().n, name='adv_out')
                ...
                self.q, self.w['q_w'], self.w['q_b'] = linear(self.l4, self.env.get_action_space().n, name='q')

            self.q_action = tf.argmax(self.q, axis=1)

            q_summary = []
            avg_q = tf.reduce_mean(self.q, 0)
            for idx in range(self.env.get_action_space().n):
        ...
        with tf.variable_scope('target'):
            ...
            if self.dueling:
                ...
                self.t_advantage, self.t_w['adv_w_out'], self.t_w['adv_w_b'] = \
                    linear(self.t_adv_hid, self.env.get_action_space().n, name='target_adv_out')
                ...
                self.target_q, self.t_w['q_w'], self.t_w['q_b'] = \
                        linear(self.target_l4, self.env.get_action_space().n, name='target_q')
        ...
        with tf.variable_scope('optimizer'):
            ...
            action_one_hot = tf.one_hot(self.action, self.env.get_action_space().n, 1.0, 0.0, name='action_one_hot')
