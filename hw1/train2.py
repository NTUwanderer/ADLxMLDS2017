...
with tf.Session() as session:
    ...
    while step < epoch:
        ...
        shuffledFiles = np.zeros([0, num_steps, numOfFeatures])
        shuffledLabels = np.zeros([0, num_steps])
        currentIndex = 0

        batch_count = 0
        batch_round = int(total_length/tempBatchSize)
        while currentIndex < numOfFiles:
            index = sequence[currentIndex]
            group = files[index]
            label = labels[index]
            newFiles = np.zeros([len(group) - num_steps + 1, num_steps, numOfFeatures])
            newLabels = np.zeros([len(group) - num_steps + 1, num_steps])
            for i in range(len(group) - num_steps + 1):
                newFiles[i] = group[i:i+num_steps]
                newLabels[i] = label[i:i+num_steps]

            if len(shuffledFiles) == 0:
                shuffledFiles = newFiles
                shuffledLabels = newLabels
            else:
                shuffledFiles = np.concatenate((shuffledFiles, newFiles))
                shuffledLabels = np.concatenate((shuffledLabels, newLabels))
        
            count = 0

            rnd = int(shuffledFiles.shape[0] / tempBatchSize)
            while (count+1) <= rnd:
                tempFiles = np.split(shuffledFiles, [tempBatchSize, len(shuffledFiles)])
                tempLabels = np.split(shuffledLabels, [tempBatchSize, len(shuffledLabels)])
                fbanks = tempFiles[0]
                label = tempLabels[0]
                shuffledFiles = tempFiles[1]
                shuffledLabels = tempLabels[1]
