...
def get_video_train_data(video_label_path):
...
def train():
    train_captions = get_video_train_data(video_train_label_path)
    ...
    for epoch in range(0, n_epochs):
        ...
        for start, end in zip(
            ...
            print('idx: ', start, " Epoch: ", epoch, " loss: ", loss_val, 'm loss: ', np.mean(loss_to_draw), ' Elapsed time: ', str((time.time() - start_time)))
        ...
        if np.mod(epoch, 10) == 0:
            ...
            saver.save(sess, os.path.join(model_path, 'model_' + str(epoch)), global_step=epoch)
