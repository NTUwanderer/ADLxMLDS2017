...
class GAN:
    ...
    def build_model(self):
        ...
        t_wrong_caption = tf.placeholder('float32', [self.params['batch_size'], self.params['caption_length']], name='wrong_caption_input')
        ...
        disc_fw_image, disc_fw_image_logits = self.discriminator(fake_image, t_wrong_caption, reuse = True)
        ...
        d_loss4 = tf.reduce_mean(disc_fw_image_logits)
        ...
        return input_tensors , variables , outputs , loss , self.clip_discriminator_var_op
    ...
    def discriminator(self , image , t_text_embedding ,reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            self.w_d = [w for w in tf.trainable_variables() if w.name.startswith('discriminator')]
            print('w: ', self.w_d)   
            self.clip_discriminator_var_op = [w.assign(tf.clip_by_value(w, -0.01, 0.01)) for w in self.w_d]
